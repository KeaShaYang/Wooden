...
public static class ExcelDataReader
{
    ...
    //自动生成C#类文件路径
    static string excelCodePath = Application.dataPath + "/Scripts/Excel/";
    //自动生成Asset文件路径
    ...
    public static void ReadOneExcelToCode(string excelFileFullPath)
    {
        ...
        if (excelMediumData != null)
        {
            ...
            {
                //写文件，生成CSharp.cs
                if (WriteCodeStrToSave(excelCodePath, excelMediumData.excelName, classCodeStr))
                {
    ...
    private static ExcelMediumData CreateClassCodeByExcelPath(string excelFileFullPath)
    {
        ...
        while (excelReader.Read())
        {
            ...
            if (curRowIndex >= excelDataRow)
            {
                ...
                {
                    //判断长度
                    if (null != propertyNameTypes[j].Key)
                    {                
                        if (j < datas.Length)
        ...
        for (int i = 0; i < propertyCount; i++)
        {
            if (null != propertyNameTypes[i].Key)
            {
                //数据名重复，数据无效
