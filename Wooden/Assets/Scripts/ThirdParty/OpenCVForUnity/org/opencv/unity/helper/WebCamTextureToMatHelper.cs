...
namespace OpenCVForUnity.UnityUtils.Helper
{
    ...
    public class WebCamTextureToMatHelper : MonoBehaviour
    {
        ...
        protected int _requestedWidth = 960;
        ...
        protected virtual void Update()
        {
            if (hasInitDone)
            {
                ...
                if (screenOrientation != Screen.orientation)
                {
                    ...
                    // 计算 显示大小
                    var size = calRelTextureSize();
                    baseMat = new Mat((int)size.y, (int)size.x, CvType.CV_8UC4);

                    if (webCamTexture.width != baseMat.width() || webCamTexture.height != baseMat.height())
                    {// 初始化
                        resizeMat = new Mat(webCamTexture.height, webCamTexture.width, CvType.CV_8UC4);
                    }
        ...
        private Vector2 calRelTextureSize() {
            Vector2 result=Vector2.zero;
            // 截取中间
            int max = Math.Max(requestedWidth, requestedHeight);
            int min = Math.Min(requestedWidth, requestedHeight);

            float requestAspect = max / (float)min;

            float textureAspect = webCamTexture.width / (float)webCamTexture.height;

            if (IsEqual(requestAspect, textureAspect))
            {
                result.x = webCamTexture.width;
                result.y = webCamTexture.height;
            }
            else if (requestAspect > textureAspect)
            {
                result.x = webCamTexture.width;
                result.y = webCamTexture.width / requestAspect;
            }
            else
            {
                result.x = requestAspect * webCamTexture.height;
                result.y = webCamTexture.height;
            }
            return result;
        }
        ...
        Mat resizeMat;
        CoreModule.Rect subRect = new CoreModule.Rect();
        ...
        public virtual Mat GetMat()
        {
            ...
            if (webCamTexture.width != baseMat.width() || webCamTexture.height != baseMat.height())
            {// 适配处理
                Utils.webCamTextureToMat(webCamTexture, resizeMat, colors, false);

                subRect.x = (int)((webCamTexture.width - baseMat.width()) * 0.5f);
                subRect.y = (int)((webCamTexture.height - baseMat.height()) * 0.5f);
                subRect.width = baseMat.width();
                subRect.height = baseMat.height();

                using (Mat subMat = resizeMat.submat(subRect)) {
                    subMat.copyTo(baseMat);
                }
