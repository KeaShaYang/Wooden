...
namespace OpenCVForUnity.UnityUtils
{
    public static class Utils
    {
        ...
        */
        ...
        public static void texture2DToMatFast(Texture2D texture2D, Mat mat, bool flip = true, int flipCode = 0)
        {
#if UNITY_EDITOR
            if (texture2D == null)
                throw new ArgumentNullException("texture2D");
            ...
            if (mat.cols() != texture2D.width || mat.rows() != texture2D.height)
                throw new ArgumentException("The Mat object must have the same size. (" + mat.cols() + "," + mat.rows() + ") - (" + texture2D.width + "," + texture2D.height + ") - (" + Screen.width + "," + Screen.height + ")");
