...
namespace Tool.UGUIExtend.Scripts
{
    ...
    /// </summary>
    [AddComponentMenu("UI/My Scroll Rect", 38)]
    [SelectionBase]
    [ExecuteAlways]
    ...
    public class MyScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        [SerializeField]
        private bool m_IsBarOnlyShowOnDraing = false;
        ...
        // 扩展功能：内容未超框时候自动锁住滑动
        public bool m_AutoLockScroll = true;
        ...
        public MovementType movementType {
            get
            {
                if (m_AutoLockScroll && m_ViewBounds != null && m_ContentBounds != null)
                {
                    Vector3 excess = m_ViewBounds.size - m_ContentBounds.size;
                    double x = Math.Round(excess.x, 1);
                    double y = Math.Round(excess.y, 1);
                    if (m_MovementType == MovementType.Elastic && ((m_Vertical && y >= 0) || (m_Horizontal && x >= 0)))
                    {
                        return MovementType.Clamped;
                    }
                }
        ...
        private bool hScrollingNeeded
        {
            get
            {
                if (Application.isPlaying)
                    return (m_ContentBounds.size.x > m_ViewBounds.size.x + 0.01f) && !(m_IsBarOnlyShowOnDraing && !m_Dragging && Mathf.Abs (m_Velocity.x) <= veAccuracy);
                return true;
            }
        }
        private bool vScrollingNeeded
        {
            ...
            {
                if (Application.isPlaying)
                    return (m_ContentBounds.size.y > m_ViewBounds.size.y + 0.01f) && !(m_IsBarOnlyShowOnDraing && !m_Dragging && Mathf.Abs(m_Velocity.y) <= veAccuracy);
                return true;
            }
        }
        ...
        {
            return InternalCalculateOffset(ref m_ViewBounds, ref m_ContentBounds, m_Horizontal, m_Vertical, movementType, ref delta);
        }
