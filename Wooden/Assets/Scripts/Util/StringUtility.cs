...
public static class StringUtility
{
    ...
    #region 转中文
    public static string toChieseNumber(long number)
    {
        string str = number.ToString();
        if (number >= 100000000)
        {
            number = number / 10000;
            str = string.Format("{0}亿", number);
        }
        else if (number >= 10000)
        {
            number = number / 10000;
            str = string.Format("{0}万", number);
        }
        return str;
    }
    ...
    public static string GetRandomString(int length)
    {
        ...
        {
            builder.Append(abc[UnityEngine.Random.Range(0, abc.Length)]);
        }
    ...
    public static Dictionary<int, int> StringToDictionary(string arr, char sp = ';', char sp2 = '_')
    {
        Dictionary<int, int> dic = new Dictionary<int, int>();
        if (!string.IsNullOrEmpty(arr))
        { 
        string[] strs = StringToStringArray(arr, sp);
            for (int i = 0; i < strs.Length; i++)
            {
                int[] s2 = StringToIntArray(strs[i], sp2);
                if (s2.Length > 1)
                {
                    if (!dic.ContainsKey(s2[0]))
                        dic.Add(s2[0], s2[1]);
                    else
                        dic[s2[0]] = s2[1];
                }
    ...
    public static Dictionary<int, string> StringTointStringDictionary(string arr, char sp = ';', char sp2 = '_')
    {
        ...
        if (!string.IsNullOrEmpty(arr))
        {
            ...
            for (int i = 0; i < strs.Length; i++)
            {
                string[] s2 = StringToStringArray(strs[i], sp2);
                if (s2.Length > 1)
                {
                    int key = 0;
                    if (int.TryParse(s2[0],out key))
                    {
    ...
    public static Dictionary<int, int[]> StringToIntArrDic(string arr, char sp = ';', char sp2 = '=',char sp3 = '_')
    {
        Dictionary<int, int[]> dic = new Dictionary<int, int[]>();
        if (!string.IsNullOrEmpty(arr))
        {
            ...
            for (int i = 0; i < strs.Length; i++)
            {
                string[] vas = StringToStringArray(strs[i],sp2);
                int key = int.Parse(vas[0]);
                if (vas.Length > 0)
                {
                    int[] s2 = StringToIntArray(vas[1], sp3);
                    if (s2.Length > 1)
                    {
                        if (!dic.ContainsKey(key))
                            dic.Add(key, s2);
    ...
    public static Dictionary<int, float[]> StringToFloatArrDic(string arr, char sp = ';', char sp2 = '_')
    {
        ...
        if (!string.IsNullOrEmpty(arr))
        {
            ...
            float[] s2 = StringToFloatArray(strs[1], sp2);
            if (s2.Length > 1)
            {
                if (!dic.ContainsKey(key))
                    dic.Add(key, s2);
